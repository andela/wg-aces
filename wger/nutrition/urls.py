...
patterns_plan = [
    ...
    url(r'^(?P<pk>\d+)/edit/$',
        ...
        name='edit'),
    url(r'^(?P<id>\d+)/pdf/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})$', # noqa E501
        plan.export_pdf,
        name='export-pdf'),
    url(r'^(?P<id>\d+)/pdf/$',
        plan.export_pdf,
...
patterns_meal_item = [
    ...
    url(r'^(?P<pk>\d+)/edit/$',
        ...
        name='edit'),
    url(r'^(?P<meal_id>\d+)/add_actual/$',
        login_required(meal_item.ActualMealItemAddView.as_view()),
        name='add_actual'),
    url(r'^(?P<pk>\d+)/edit_actual/$',
        login_required(meal_item.ActualMealItemEditView.as_view()),
        name='edit_actual'),
