...
def pagination(paginator, page):
    ...
    # items) we muck around here to remove the pages not inmediately 'around'
    # the current one, otherwise we end up with a useless block with 300 pages.
...
def auto_link_css(flavour='full', css=''):
    '''
    Adds the appropriate classes to a sidebar link depending
    on the site version
...
def is_checkbox(field):
    '''
    Tests if a field element is a checkbox, as it needs to be
