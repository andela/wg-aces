...
class UserProfile(models.Model):
    ...
    # (obviously this is only meaningful if
    # the user has a language other than english)
...
'''),
    ...
    calories = IntegerField(verbose_name=_('Total daily calories'),
                            help_text=_('Total caloric intake, including\
                            e.g. any surplus'),
                            default=2500,
                            blank=False,
                            null=True,
...
class DaysOfWeek(models.Model):
    '''
    Model for the days of the week. This model is needed so that
    'Day' can have multiple days of the week selected
    '''
    TRAINING_CYCLES = (
        ('microcycle', 'Micro'),
        ('mesocycle', 'Meso'),
    ...
    training_cycle = models.CharField(
        max_length=10, choices=TRAINING_CYCLES, default="microcycle")
...
@python_2_unicode_compatible
class LicenseAuthor(models.Model):
    '''
    License for an item (exercise, ingredient, etc.)
    '''
