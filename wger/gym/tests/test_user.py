...
class GymAddUserTestCase(WorkoutManagerTestCase):
    ...
    def add_user(self, fail=False):
        ...
        if fail:
            ...
            self.assertEqual(len(self.client.session['gym.user']
                                 ['password']), 15)
            new_user = User.objects.get(pk=self.client.session['gym.user']
                                        ['user_pk'])
    ...
    def new_user_data_export(self, fail=False):
        ...
        if fail:
            ...
            data = 'User-data-{t.year}-{t.month:02d}-{t.day:02d}-cletus.csv'
            filename = data.format(t=today)
