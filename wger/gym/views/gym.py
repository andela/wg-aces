...
def gym_new_user_info(request):
    ...
    context = {'new_user': get_object_or_404(
        User,
...
def gym_new_user_info_export(request):
    ...
    writer.writerow([_('Username'), _('First name'),
                     _('Last name'), _('Gym'), _('Password')])
    ...
    data = 'User-data-{t.year}-{t.month:02d}-{t.day:02d}-{user}.csv'
    filename = data.format(t=today, user=new_username)
