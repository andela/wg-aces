...
class WorkoutLogCacheTestCase(WorkoutManagerTestCase):
    ...
    def test_calendar_day(self):
        '''
        Test the log cache on the calendar day view
        is correctly generated on visit
        '''
        ...
        self.client.get(reverse(
            'manager:workout:calendar-day', kwargs={'username': 'admin',
                                                    'year': 2012,
                                                    'month': 10,
                                                    'day': 1}))
        self.assertTrue(cache.get(cache_mapper.get_workout_log_list(log_hash)))
    ...
    def test_cache_delete_log(self):
        ...
        self.assertFalse(cache.get(cache_mapper.get_workout_log_list(
            log_hash)))
        self.assertFalse(cache.get(cache_mapper.get_workout_log_list(
            log_hash_day)))
    ...
    def test_cache_delete_log_2(self):
        ...
        self.assertTrue(cache.get(cache_mapper.get_workout_log_list(
            log_hash)))
        self.assertTrue(cache.get(cache_mapper.get_workout_log_list(
            log_hash_day)))
