...
class ScheduleModelTestCase(WorkoutManagerTestCase):
    ...
    def test_get_workout_steps_test_4(self):
        '''
        Test with 3 steps. Start is too far in the
        past, schedule ist not a loop
        '''
        self.user_login('test')
        user = User.objects.get(pk=2)
        self.delete_objects(user)
...
class SchedulePdfExportTestCase(WorkoutManagerTestCase):
    ...
    def export_pdf_token(self, pdf_type="log"):
        ...
        self.assertEqual(response['Content-Disposition'],
                         'attachment; filename=Schedule-1-{0}.pdf'.format(
                             pdf_type))
    ...
    def export_pdf_with_comments(self, fail=False, pdf_type="log"):
        ...
        as a pdf, with exercise coments
        '''
        ...
        if fail:
            ...
            self.assertEqual(response['Content-Type'], 'application/pdf')
            self.assertEqual(response['Content-Disposition'],
                             'attachment; filename=Schedule-3-{0}.pdf'.format(
                                 pdf_type))
    ...
    def export_pdf_with_images(self, fail=False, pdf_type="log"):
        '''
        Helper function to test exporting a workout
        as a pdf, with exercise images
        '''
        user = User.objects.get(username='test')
        uid, token = make_token(user)
        response = self.client.get(reverse('manager:schedule:pdf-{0}'.format(
            pdf_type),
            kwargs={'pk': 3,
                    'images': 1,
                    'comments': 0,
    ...
    def test_export_pdf_log_with_images_and_comments(self, fail=False):
        '''
        Tests exporting a workout as a pdf as the
        owner user with images and comments
        '''
        self.user_login('test')
