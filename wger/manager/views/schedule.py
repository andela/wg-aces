...
def export_pdf_log(request, pk, images=False, comments=False, uidb64=None,
                   token=None):
    '''
    Show the workout schedule
    '''
    ...
    doc.build(elements)
    data = 'attachment; filename=Schedule-{0}-log.pdf'
    response['Content-Disposition'] = data.format(pk)
    response['Content-Length'] = len(response.content)
    return response


def export_pdf_table(request, pk, images=False, comments=False, uidb64=None,
                     token=None):
    '''
    Show the workout schedule
    '''
    ...
    # write the document and send the response to the browser
    doc.build(elements)
    data = 'attachment; filename=Schedule-{0}-table.pdf'
    response['Content-Disposition'] = data.format(pk)
    response['Content-Length'] = len(response.content)
    return response
