...
class DayView(WgerFormMixin, LoginRequiredMixin):
    ...
    def get_day_key(self, key):
        keys = {
            'microcyle': 'Micro',
            'mesocycle': 'Meso',
            'macrocycle': 'Macro'
        }
        return keys.get(key, 'Micro')
        ...
        for day in workout.day_set.all():
            for weekday in day.day.all():
                if not self.object or day.id != self.object.id:
                    used_days.append(weekday.id)
        used_days.sort()
        queryset = DaysOfWeek.objects.filter(
            training_cycle=self.get_day_key(workout.cycle_kind)).exclude(
            id__in=used_days).all()
        context['form'].fields['day'].queryset = queryset
        label = self.get_cycle(workout.cycle_kind)
        context['form'].fields['description'].help_text = \
            _('A description of what is done for this {0} (e.g. '
              '"Pull {0}") or what body parts are trained (e.g. '
              "Arms and abs"')'.format(label))
...
class DayCreateView(DayView, CreateView):
    ...
    title = ugettext_lazy('Add workout cycle')
