...
def workout_log(request, id, images=False, comments=False, uidb64=None,
    ...
    * http://www.reportlab.com/apis/reportlab/dev/platypus.html
    ...
    elements.append(Spacer(10 * cm, 0.5 * cm))
    elements.append(render_footer(request.build_absolute_uri(
    ...
    # Create the HttpResponse object with the appropriate PDF headers.
    data = 'attachment; filename=Workout-{0}-log.pdf'
    response['Content-Disposition'] = data.format(id)
...
def workout_view(request, id, images=False, comments=False, uidb64=None,
    ...
    * http://www.reportlab.com/apis/reportlab/dev/platypus.html
    ...
    # Create the HttpResponse object with the appropriate PDF headers.
    data = 'attachment; filename=Workout-{0}-table.pdf'
    response['Content-Disposition'] = data.format(id)
    response['Content-Length'] = len(response.content)
    return response
